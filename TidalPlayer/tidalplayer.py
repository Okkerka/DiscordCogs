...
class TidalPlayer(commands.Cog):
    ...
    async def _check_ready(self,ctx):
        if not TIDALAPI_AVAILABLE:
            return await ctx.send("❌ Install tidalapi"),False
        if not self.session or not self.session.check_login():
            return await ctx.send("❌ Run >tidalsetup"),False
        if not self.bot.get_cog("Audio"):
            return await ctx.send("❌ Audio cog missing"),False
        return True,None

    def _suppress_enqueued(self,ctx):
        if hasattr(ctx,"_orig"): return
        ctx._orig=ctx.send
        async def o(*a,**k):
            e=k.get("embed") or (a[0] if a and isinstance(a[0],discord.Embed) else None)
            if e and "Track Enqueued" in getattr(e,"title",""): return
            return await ctx._orig(*a,**k)
        ctx.send=o

    def _restore_send(self,ctx):
        if hasattr(ctx,"_orig"):
            ctx.send=ctx._orig
            delattr(ctx,"_orig")

    @commands.command(name="tplay")
    async def tplay(self,ctx,*,q:str):
        ok,err=await self._check_ready(ctx)
        if not ok: return
        quiet=await self.config.quiet_mode()
        if quiet: self._suppress_enqueued(ctx)
        try:
            if SPOTIFY_AVAILABLE and "spotify.com/playlist/" in q:
                await self._queue_spotify(ctx,q)
            elif YOUTUBE_API_AVAILABLE and "list=" in q:
                await self._queue_youtube(ctx,q)
            elif "tidal.com" in q or re.search(r"(playlist|album|track|mix)/",q):
                await self._handle_tidal(ctx,q)
            else:
                await self._search_play(ctx,q)
        finally:
            if quiet: self._restore_send(ctx)

    async def _search_play(self,ctx,query):
        async with ctx.typing():
            res=await self.bot.loop.run_in_executor(None,self.session.search,query)
        tracks=res.get("tracks",[])
        if not tracks: return await ctx.send("❌ No tracks.")
        await self._play(ctx,tracks[0])

    async def _handle_tidal(self,ctx,url):
        kind=("playlist" if "playlist/" in url else
              "album"    if "album/"    in url else
              "mix"      if "mix/"      in url else
              "track")
        m=re.search(rf"{kind}/([A-Za-z0-9\-]+)",url)
        if not m: return await ctx.send("❌ Invalid Tidal URL")
        obj=await self.bot.loop.run_in_executor(None,getattr(self.session,kind),m.group(1))
        ...
        msg=await ctx.send(f"⏳ Queueing Tidal {kind} '{name}'({len(items)})…")
        for t in items: await self._play(ctx,t,show_embed=False)
        await msg.edit(content=f"✅ Queued {len(items)} from **{name}**")

    async def _queue_spotify(self,ctx,url):
        m=re.search(r"playlist/([A-Za-z0-9]+)",url)
        if not m: return await ctx.send("❌ Invalid Spotify URL")
        await self.config.spotify_client_id()  # ensure config
        await self.config.spotify_client_secret()
        self.sp=spotipy.Spotify(SpotifyClientCredentials(
            await self.config.spotify_client_id(),
            await self.config.spotify_client_secret()
        ))
        pid=m.group(1)
        items=self.sp.playlist_items(pid)["items"]
        await ctx.send(f"⏳ Queuing Spotify({len(items)})…")
        for it in items:
            t=it["track"]
            await self._search_play(ctx,f"{t['artists'][0]['name']} {t['name']}")
        await ctx.send("✅ Done.")

    async def _queue_youtube(self,ctx,url):
        m=re.search(r"list=([A-Za-z0-9_-]+)",url)
        if not m: return await ctx.send("❌ Invalid YouTube URL")
        pid=m.group(1)
        videos=[];req=self.yt.playlistItems().list(part="snippet",playlistId=pid,maxResults=50)
        while req:
            res=req.execute()
            videos+= [it["snippet"]["title"] for it in res["items"]]
            req=self.yt.playlistItems().list_next(req,res)
        await ctx.send(f"⏳ Queuing YouTube({len(videos)})…")
        for title in videos: await self._search_play(ctx,title)
        await ctx.send("✅ Done.")

    @commands.Cog.listener()
    async def on_player_stop(self,player):
        g=self.bot.get_guild(int(player.guild_id))
        if g: await self._pop_meta(g.id)

    @commands.command(name="tqueue",aliases=["q"])
    async def tqueue(self,ctx):
        data=await self.config.guild(ctx.guild).track_metadata()
        if not data: return await ctx.send("Queue empty")
        pages=[] 
        for i in range(0,len(data),10):
            ch=data[i:i+10]
            desc="\n".join(f"`{i+j+1}.` **{m['title']}** • {m['artist']} • `{self._format_time(m['duration'])}`"
                          for j,m in enumerate(ch))
            pages.append(discord.Embed(title="Queue",description=desc,color=discord.Color.green()))
    ...
    async def tidalsetup(self,ctx):
        if not TIDALAPI_AVAILABLE:
            return await ctx.send("❌ Install tidalapi")
        login,fut=self.session.login_oauth()
        embed=discord.Embed(title="Tidal OAuth",description=login.verification_uri_complete)
        await ctx.send(embed=embed)
        try: await asyncio.wait_for(self.bot.loop.run_in_executor(None,fut.result),timeout=300)
        except asyncio.TimeoutError: return await ctx.send("⏱️ Timeout")
        ...
        else: await ctx.send("❌ Login failed")
    ...
    async def spotify_setup(self,ctx,client_id:str,client_secret:str):
        if not SPOTIFY_AVAILABLE:
            return await ctx.send("❌ Install spotipy")
        await self.config.spotify_client_id.set(client_id)
        await self.config.spotify_client_secret.set(client_secret)
        self.sp=spotipy.Spotify(SpotifyClientCredentials(client_id,client_secret))
    ...
    async def tidalquiet(self,ctx,mode:str=None):
        ...
        if mode is None:
            st="enabled" if await self.config.quiet_mode() else "disabled"
            return await ctx.send(f"Quiet mode {st}")
