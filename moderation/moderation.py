...
def mod_or_permissions(**perms):
    async def predicate(ctx: commands.Context):
        ...
        if await ctx.bot.is_owner(ctx.author) or (ctx.guild and ctx.author == ctx.guild.owner):
            return True
...
class Moderation(commands.Cog):
        ...
        while not self.bot.is_closed():
            try:
                for guild in self.bot.guilds:
                    ...
                    now = datetime.utcnow()
                    expired_ids = [
                        int(uid) for uid, ban_info in tempbans.items()
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: Optional[str] = None):
        ...
        try:
            ...
            await member.kick(reason=f"By {ctx.author}: {reason or 'No reason provided'}")
    ...
    async def ban(self, ctx, user: Union[discord.Member, int], delete_days: int = 1, *, reason: Optional[str] = None):
        ...
        try:
            ...
            await ctx.guild.ban(user, reason=f"By {ctx.author}: {reason or 'No reason provided'}", delete_message_days=delete_days)
    ...
    async def unban(self, ctx, user_id: int, *, reason: Optional[str] = None):
        ...
        try:
            user = await self.bot.fetch_user(user_id)
            await ctx.guild.unban(user, reason=f"By {ctx.author}: {reason or 'No reason provided'}")
    ...
    @mod_or_permissions(ban_members=True)
    @commands.bot_has_permissions(ban_members=True)
    async def softban(self, ctx, member: discord.Member, *, reason: Optional[str] = None):
        ...
        try:
            ...
            await ctx.guild.ban(member, reason=f"Softban by {ctx.author}: {reason or 'No reason provided'}", delete_message_days=1)
    ...
    @mod_or_permissions(ban_members=True)
    @commands.bot_has_permissions(ban_members=True)
    async def tempban(self, ctx, user: Union[discord.Member, int], duration: str, *, reason: Optional[str] = None):
        ...
        try:
            ...
            await ctx.guild.ban(user, reason=f"Tempban by {ctx.author}: {reason or 'No reason provided'}", delete_message_days=1)
    ...
    async def timeout(self, ctx, member: discord.Member, duration: str, *, reason: Optional[str] = None):
        ...
        try:
            await member.timeout(delta, reason=f"By {ctx.author}: {reason or 'No reason provided'}")
    ...
    async def untimeout(self, ctx, member: discord.Member, *, reason: Optional[str] = None):
        ...
        try:
            await member.timeout(None, reason=f"By {ctx.author}: {reason or 'No reason provided'}")
    ...
    async def voicekick(self, ctx, member: discord.Member, *, reason: Optional[str] = None):
        ...
        try:
            await member.move_to(None, reason=f"Voice kicked by {ctx.author}: {reason or 'No reason provided'}")
        ...
        async def voicemute(self, ctx, member: discord.Member, *, reason: Optional[str] = None):
         """Server mute a member in voice channels."""
            if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                await ctx.send("❌ Cannot voice mute someone with equal or higher role.")
                return
            ...
            try:
                await member.edit(mute=True, reason=f"Voice muted by {ctx.author}: {reason or 'No reason provided'}")
        ...
        @mod_or_permissions(mute_members=True)
        @commands.bot_has_permissions(mute_members=True)
        async def voiceunmute(self, ctx, member: discord.Member, *, reason: Optional[str] = None):
            ...
            try:
                await member.edit(mute=False, reason=f"Voice unmuted by {ctx.author}: {reason or 'No reason provided'}")
    ...
    async def voiceban(self, ctx, member: discord.Member, *, reason: Optional[str] = None):
        ...
        try:
            await member.edit(mute=True, deafen=True, reason=f"Voice banned by {ctx.author}: {reason or 'No reason provided'}")
    ...
    @mod_or_permissions(mute_members=True, deafen_members=True)
    @commands.bot_has_permissions(mute_members=True, deafen_members=True)
    async def voiceunban(self, ctx, member: discord.Member, *, reason: Optional[str] = None):
        """Unban a member from speaking and listening in voice channels."""
        try:
            await member.edit(mute=False, deafen=False, reason=f"Voice unbanned by {ctx.author}: {reason or 'No reason provided'}")
