...
def parse_timestamp(value: str, zone: str) -> int:
    ...
    try:
        ...
        raise ValueError(
            "Use YYYY-MM-DD HH:MM and an IANA timezone such as Europe/Budapest."
        ) from exc
    if dt.tzinfo is None:
        first, second = dt.replace(tzinfo=tz, fold=0), dt.replace(tzinfo=tz, fold=1)
        if first.utcoffset() != second.utcoffset():
            raise ValueError(
                "This local time is ambiguous or does not exist. Supply an explicit UTC offset."
            )
        if first.astimezone(timezone.utc).astimezone(tz).replace(tzinfo=None) != dt:
            raise ValueError(
                "This local time does not exist. Supply an explicit UTC offset."
            )
        dt = first
