...
class ConfigModal(discord.ui.Modal, title="ChatTriggers Configuration"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await self.cog.config.guild(interaction.guild).trigger_phrase.set(self.trigger_phrase.value)
        await self.cog.config.guild(interaction.guild).gif_url.set(self.gif_url.value)
        await self.cog.config.guild(interaction.guild).sound_url.set(self.sound_url.value)
...
class UserSelectView(discord.ui.View):
    ...
    @discord.ui.select(cls=discord.ui.UserSelect, placeholder="Select allowed users...", min_values=0, max_values=25)
    async def select_users(self, interaction: discord.Interaction, select: discord.ui.UserSelect):
        ...
        await interaction.response.send_message(f"✅ Added {len(new_ids)} users to allowlist.", ephemeral=True)
...
class MainView(discord.ui.View):
    ...
    @discord.ui.button(label="Config Text/Media", style=discord.ButtonStyle.primary, emoji="⚙️")
    async def config_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(ConfigModal(self.cog))

    @discord.ui.button(label="Add Users (GUI)", style=discord.ButtonStyle.secondary, emoji="👥")
    async def users_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("Select users to ADD:", view=UserSelectView(self.cog), ephemeral=True)
...
class ChatTriggers(commands.Cog):
        ...
        try:
            ...
            try:
                ...
                if player:
                    ...
                    # Force move to user's channel (Safe Fix)
            ...
            # 4. Visual Nuke
            if settings['gif_url']:
                embed = discord.Embed(title="🚨 ALERT TRIGGERED 🚨", color=discord.Color.red())
                embed.set_image(url=settings['gif_url'])
                await channel.send(content=f"## 🚨 ALERT TRIGGERED BY {user.mention} 🚨", embed=embed)
    ...
    async def on_message(self, message):
        ...
        if phrase and phrase.lower() in message.content.lower():
            ...
            # Added Owner Bypass for easier testing
            if message.author.id in allowed or await self.bot.is_owner(message.author):
