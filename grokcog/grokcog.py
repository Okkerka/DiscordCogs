...
class GrokCog(commands.Cog):
    ...
    async def _validate(
        ...
        if not question or not question.strip():
            await channel.send("âŒ Please provide a question.")
            return False
        if guild_id:
            guild_config = self.config.guild_from_id(guild_id)
            if not await guild_config.enabled():
                await channel.send("âŒ Grok is disabled in this server.")
                return False
        ...
        if len(question) > max_length:
            await channel.send(
                f"âŒ Question too long ({len(question)}/{max_length} characters). "
                "Please shorten it."
            )
            return False
    ...
    async def _ask_groq(self, question: str, temperature: float) -> dict:
        ...
        if not api_key:
            raise ValueError(
                "âŒ **Groq API key not configured!**\n\n"
            ...
            try:
                ...
                async with self._session.post(
                    ...
                    if resp.status == 429:
                        ...
                        raise ValueError(
                            "âŒ **401 Unauthorized** - Your Groq API key is invalid.\n\n"
                        ...
                        raise ValueError(f"âŒ **Bad Request**: {error_msg}")
                        ...
                        if attempt < max_retries - 1:
                            ...
                            continue
                        raise ValueError(
                            f"âŒ Groq server error (HTTP {resp.status}). Please try again later."
                        )
                ...
                if attempt == max_retries - 1:
                    raise ValueError(f"âŒ Connection failed: {str(e)}")
                ...
                if attempt == max_retries - 1:
                    raise
                await asyncio.sleep(min(2**attempt, 15))
        raise ValueError("âŒ Failed after all retry attempts")
    ...
    async def _process(
        ...
        try:
            ...
            await channel.send(
                str(e) if isinstance(e, ValueError) else "âŒ Unexpected error"
            )
        finally:
    ...
    def _format(self, data: dict) -> discord.Embed:
        ...
        embed.set_footer(
            text="Powered by 2 Romanian kids â€¢ Retardation only (fact-checks)"
        )
    ...
    async def admin_apikey(self, ctx: commands.Context, *, api_key: str):
        ...
        if len(api_key) < 32:
            await ctx.send("âŒ Invalid API key format (too short)")
            return
    ...
    async def admin_verify(self, ctx: commands.Context):
        msg = await ctx.send("ðŸ” Testing API connection...")
        try:
            test_payload = {
            ...
            async with self._session.post(
                ...
                if resp.status == 200:
                    ...
                    error_msg = {
                        401: "âŒ Invalid API key",
                        403: "âŒ No access to this model",
            ...
            await msg.edit(content=f"âŒ Error: {str(e)}")
    ...
    async def admin_cooldown(self, ctx: commands.Context, seconds: int):
        if seconds < 5:
            await ctx.send("âŒ Cooldown must be at least 5 seconds")
            return
    ...
    async def admin_setmodel(self, ctx: commands.Context, model: str):
        ...
        if model not in valid_models:
            await ctx.send(
                f"âŒ Invalid model. Choose from:\n"
                + "\n".join(f"â€¢ `{m}`" for m in valid_models)
            )
            return
    ...
    async def admin_ratelimits(
        ...
        if per_minute < 1:
            await ctx.send("âŒ Must allow at least 1 request per minute")
            return
        if min_gap < 0.1:
            await ctx.send("âŒ Minimum gap must be at least 0.1 seconds")
            return
    ...
    async def admin_diagnose(self, ctx: commands.Context):
        await ctx.typing()
        embed = discord.Embed(
            title="ðŸ› ï¸ GrokCog Diagnostics",
            color=discord.Color.blue(),
            timestamp=datetime.now(timezone.utc),
        )
        api_key = await self.config.api_key()
        embed.add_field(
            name="API Key Set", value="âœ… Yes" if api_key else "âŒ No", inline=True
        )
        model = await self.config.model_name()
        ...
        embed.add_field(
            name="Request Queue",
            value="âœ… Enabled" if queue_enabled else "âŒ Disabled",
            inline=True,
        )
        if self._last_api_call:
            time_since = (
